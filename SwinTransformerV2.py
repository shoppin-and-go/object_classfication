...
def train_SwinTransformerV2(config):
    ...
    print(f"Model: {config.model_name}, Total params: {sum(p.numel() for p in model.parameters())}")
    ...
    for epoch in range(config.epochs):
        ...
        for inputs, labels_batch, image_paths in train_loader:
        ...
        with torch.no_grad():
            for inputs, labels_batch, image_paths in val_loader:
                inputs = inputs.to(config.device)
                labels_batch = labels_batch.to(config.device)
...
def train_SwinTransformerV2_with_Test(config):
    images, labels = read_images_and_labels(config)
    train_loader, val_loader = create_dataloader(config, images, labels)
    ...
    new_config: CNN_config = config
    new_config.mode = 'test'
    ...
    for epoch in range(config.epochs):
        ...
        if val_acc > best_acc:
            ...
            epochs_no_improve = 0
            test_SwinTransformerV2(new_config)
...
def test_SwinTransformerV2(config):
    images, labels = read_images_and_labels(config)
    test_loader = create_dataloader(config, images, labels)

    model = create_model(config)
    model = model.to(config.device)
    print(f"Model: {config.model_name}, Total params: {sum(p.numel() for p in model.parameters())}, device: {config.device}")
    ...
    evaluate_SwinTransformerV2(config, model, test_loader)


def evaluate_SwinTransformerV2(config, model, test_loader):
    test_corrects = 0
    total = 0
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            for img_path, pred, actual in zip(image_paths, preds, labels):
                predicted_label = config.idx_to_class[pred.item()]
                actual_label = config.idx_to_class[actual.item()]
                if predicted_label != actual_label:
                    print(f"Image: {img_path}, Predicted: {predicted_label}, Actual: {actual_label} <--- Mismatch!")
                else:
                    print(f"Image: {img_path}, Predicted: {predicted_label}, Actual: {actual_label}")

    accuracy = test_corrects.double() / total
...
if __name__ == '__main__':
    ...
    if config.mode == "train":
        ...
        # train_SwinTransformerV2_with_Test(config)
    else:
        test_SwinTransformerV2(config)
